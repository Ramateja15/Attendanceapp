 ...
 class DbHelper extends SQLiteOpenHelper {
    ...
    int deleteStudent(long sid){
     ...
     long updateStudent(long sid, String name) {
         ...
         return database.update(STUDENT_TABLE_NAME,values,C_ID+"=?",new String[]{String.valueOf(sid)});
     }
     long addStatus(long sid,long cid,String date,String status){
         ...
         long result = database.insertWithOnConflict(STATUS_TABLE_NAME,null,values,SQLiteDatabase.CONFLICT_REPLACE);
         return result;
     }
     ...
     String getStatus(long sid,String date){
        ...
        if(cursor.moveToFirst()) {
            int nen = cursor.getColumnIndex(STATUS_KEY);
            status = cursor.getString(nen);
        }
        return status;
     }
